    ...
    return {
      shaderCode: `
        ...
        float sceneSDF(vec3 samplePoint) {
            ...
            balls = unionSDF(sphereSDF(vec3(samplePoint.x + 2.0, samplePoint.y - 1.0 + 0.5 * abs(sin(iTime)), samplePoint.z) / 0.5) * 0.5, balls);
            balls = unionSDF(sphereSDF(vec3(samplePoint.x - 2.0, samplePoint.y - 1.0 + 0.5 * abs(sin(iTime)), samplePoint.z) / 0.5) * 0.5, balls);
            float torusDist = sdTorus(samplePoint, vec2(0.3,1.5 * abs(sin(iTime))));
        ...
        void main()
        {
            ...
            vec3 K_d = K_a;
            vec3 K_s = vec3(0.556,1.000,0.735);

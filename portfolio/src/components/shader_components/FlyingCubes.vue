      ...
      shaderCode: `
                ...
                */
                float sceneSDF(vec3 samplePoint) {    
                    ...
                    */ 
                    float Oct = boxSDF(samplePoint, vec3(0.8));
                    float bdBox = boxSDF(samplePoint * rotateX(iTime / 2.0), vec3(0.8, 0.8, 0.8));
                    
                    
                    float pydH = 0.5 * sin(iTime);
                    float pydOffset =  1.0 + sin(1.7*iTime) *3.0;
                    
                    float cube = boxSDF(samplePoint - vec3(pydOffset, 0.0, 0.0), vec3(pydH));
                    cube = smoothUnionSDF(cube, boxSDF(samplePoint + vec3(pydOffset, 0.0, 0.0), vec3(pydH)), 0.3);
                    cube =  smoothUnionSDF(cube, boxSDF(samplePoint - vec3(pydOffset, 0.0, 0.0), vec3(pydH)), 0.3);
                    cube =  smoothUnionSDF(cube, boxSDF(samplePoint + vec3(0.0,pydOffset, 0.0), vec3(pydH)), 0.3);// top
                    cube =  smoothUnionSDF(cube, boxSDF(samplePoint - vec3(0.0,pydOffset, 0.0), vec3(pydH)), 0.3);// bottom
                    cube =  smoothUnionSDF(cube, boxSDF(samplePoint + vec3(0.0, 0.0,pydOffset), vec3(pydH)), 0.3);
                    cube =  smoothUnionSDF(cube, boxSDF(samplePoint - vec3(0.0, 0.0, pydOffset), vec3(pydH)), 0.3);
                    
                    float radius = 0.8 * sin(iTime);
                    float box = sdBoundingBox(samplePoint, vec3(radius), 0.05);
                    
                    float finalSDF = onionSDF(smoothUnionSDF(box, onionSDF(smoothUnionSDF(smoothIntersectionSDF(Oct,bdBox, 0.1), cube, 0.1), 0.1), 0.3), 0.02);
